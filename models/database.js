...
var query8 = client.query('create table questions(askedDate timestamp not null, userID text references users(userID), subject varchar(20), question text not null, answer text, answerDate timestamp, rate varchar(2), favorite boolean, read boolean, mentorID text references users(userID), primary key(userID, askedDate))');
var query9 = client.query('create table subscription(userID text primary key references users(userID), token text, status varchar(10), type varchar(30))');
...
var query11 = client.query('create table recommendations(recomID serial unique not null primary key, location varchar(50), subject varchar(20), spanish boolean, english boolean, type varchar(10), schoolType varchar(15), format varchar(10), groupsize varchar(7), level varchar(25), mentorID text references users(userID), active boolean)');
var query12 = client.query('create table recommendation_body(recomID integer unique not null primary key references recommendations(recomID), title varchar(50), multimedia text, header varchar(100), description text)');
...
var query15 = client.query('create table recommendation_topics(recomID integer unique not null primary key references recommendations(recomID), topicA varchar(20), topicB varchar(20), topicC varchar(20))');
...
var query17 = client.query('create table quiz_question(quizQuestionID serial unique not null primary key, question varchar(500), recomID integer references recommendations(recomID))');
var query18 = client.query('create table quiz_question_choice(choiceID serial unique not null, quizQuestionID integer references quiz_question(quizQuestionID), choice varchar(300), correctAnswer boolean not null, primary key(choiceID, quizQuestionID))');
var query19 = client.query('create table quiz_question_answer(answerID serial unique not null, userID text references users(userID), choiceID integer references quiz_question_choice(choiceID), quizQuestionID integer references quiz_question(quizQuestionID), primary key(answerID, quizQuestionID))');
var query20 = client.query('create table quiz(quizID serial unique not null, userID text references users(userID), q1 integer references quiz_question(quizQuestionID),q2 integer references quiz_question(quizQuestionID), q3 integer references quiz_question(quizQuestionID), q4 integer references quiz_question(quizQuestionID), q5 integer references quiz_question(quizQuestionID), q6 integer references quiz_question(quizQuestionID), q7 integer references quiz_question(quizQuestionID), q8 integer references quiz_question(quizQuestionID),q9 integer references quiz_question(quizQuestionID),q10 integer references quiz_question(quizQuestionID), q11 integer references quiz_question(quizQuestionID),q12 integer references quiz_question(quizQuestionID), score smallint, created timestamp, primary key(quizID, userID))');
