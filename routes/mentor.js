...
router.delete('/questions/remove', (req,res,next)=> {
  ...
  pg.connect(connectionString, (err, client, done)=> {
    ...
    if(err){
     ...
     //verify if user exists in database records
     const query1 = client.query('SELECT * FROM users WHERE userid = $1 AND usertype = $2', [data.userid, 'mentor']);
     //stream results back one row at a time
     query1.on('row', (row) => {
       userexist.push(row);
     });
     query1.on('end', () => {
       ...
       if (userexist.length === 1){ // user exists and is of type mentor
        ...
        return res.status(401).json({statusCode: 401,
          message: 'User does not exists in records or is not a mentor. Inputs where not received as expected.',
          isBase64Encoded: false,});
      }
    });
  });
