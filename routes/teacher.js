    ...
    query1.on('end', () => {
      ...
      if (resultsexist.length === 1) { // user exists
        ...
        query.on('end', () => {
          ...
          if (questions.length > 0) { //there are questions
    ...
    query.on('end', () => {
      ...
      if (results.length === 1) {
        if (data.action == 'active') {
          //If the user was active it cancels its subscription and sets its value to suspended in the database
          ...
          //Gets the cancelled subscription customer id and with it tries to resubscribe with a new subscription without coupon
...
router.post('/quizzes/take', checkJwt, (req, res, next) => {
  ...
  //verify all answers are in array
  ...
  //validate all answers have both parameters
